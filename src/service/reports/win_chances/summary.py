...
from src.service.reports.win_chances.schemas import WinChancesItem, WinChancesQuery, WindowSegment


class WinChancesSummary(Summary):
    _file_output_dir = "win_chances"

    def __init__(self, query: WinChancesQuery, dataset: dict[int, WinChancesItem]) -> None:
        super().__init__(query=query, dataset=dataset)

    def _collect_windows(self, games_dict: dict[int, WinChancesItem], side: NBATeamSide):
        games_with_windows: list[list[WindowSegment]] = []
        all_windows: list[WindowSegment] = []

        for game in games_dict.values():
            windows = (  # pyright: ignore[reportUnknownVariableType]
                game._window_segs.get(side)  # pyright: ignore[reportAttributeAccessIssue, reportUnknownMemberType]
                if game._window_segs  # pyright: ignore[reportAttributeAccessIssue, reportUnknownMemberType]
                else None
            )
            if not windows:
                continue
    ...
    def _compute_stats_text(
        self, games_dict: dict[int, WinChancesItem], team: str | None = None, team_vs: str | None = None
    ) -> str:
    ...
    def _make_data_summary(self) -> Path:
        ...
        all_games: dict[int, WinChancesItem] = self._dataset
